...
VShapedAttachedKSMModel::VShapedAttachedKSMModel(const EngineParams& params)
    : KSMModel(KSMType::VShapedAttached, params) {}

void VShapedAttachedKSMModel::calculate(CylinderResults& results,
    ...
    const double tet = g - gp; // угол между главным и прицепным шатунами
    ...
    for (double a_deg : alpha) {
        ...
        // ===== БОКОВОЙ ЦИЛИНДР (прицепной) =====
        {
            ...
            // Перемещения (первая и вторая гармоники для справки)
