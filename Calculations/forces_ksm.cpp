...
<div class="box" style="margin-top:16px">
  ...
  <div class="meta">
    D = )"
    << p.diam_cyl << R"( м,  S = )" << p.stroke << R"( м,  F_p = )" << R.F_p << R"( м²,
    r = )"
    << p.r << R"( м,  L = )" << p.leng_rod << R"( м,  λ = )" << p.lyambda << R"(<br>
    ω = )"
    << p.w << R"( рад/с,  m<sub>rec</sub> = )" << R.m_rec << R"( кг,  τ = )" << p.tau << R"(
  </div>
</div>
                               ...
                               bool auto_open_html)
{
  ...
  if (lambda <= 0.0 && Lrod > 0.0)
    lambda = Rcr / Lrod;
  if (Lrod <= 0.0 && lambda > 0.0)
    Lrod = Rcr / lambda;
  ...
  auto upd_absmax = [](double &m, double v)
  ...
  for (size_t i = 0; i < N; ++i)
  {
    ...
    // защита от cosβ≈0
    if (std::fabs(cB) < 1e-8)
      cB = (cB >= 0 ? 1e-8 : -1e-8), tB = std::tan(std::acos(cB));
    double K = Fsum / cB;
    double N = Fsum * tB;
  ...
  if (!out_html.empty())
  {
    std::vector<double> Pgas_MPa;
    Pgas_MPa.reserve(N);
    for (double Fg : R.P_gas)
      Pgas_MPa.push_back(Fg / 1e6); // Н -> МПа*м² условно, но визуально сопоставимо
