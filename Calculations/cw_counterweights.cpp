                                 ...
                                 int seg)
{
    ...
    for (int i=0;i<n;i++){
        ...
        tri(f, nZp(), A2,B2,C2); tri(f, nZp(), A2,C2,D2);

        // внешняя боковая
        V3 nOut{ std::cos((a0+a1)*0.5), std::sin((a0+a1)*0.5), 0.0 };
        tri(f, nOut, B,C,C2); tri(f, nOut, B,C2,B2);

        // внутренняя боковая
        V3 nIn{ -nOut.x, -nOut.y, 0.0 };
        tri(f, nIn, D,A,A2); tri(f, nIn, D,A2,D2);
    }
...
/*
  ...
  Во всех случаях сектор расположен В ПЛОСКОСТИ ЩЁК (XY), вокруг оси Z и смотрит ВНИЗ (−Y).
*/
...
CWResult build_counterweights_and_export(
    ...
    // ---- Статический момент S_prot по ТЗ (БЕЗ *p.r!) ----
    double S = 0.0;
    switch (var){
        case CWVariant::FullSupport_V1:
