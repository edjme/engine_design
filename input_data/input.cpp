...
// trim по месту
inline void ltrim(Str& s){ s.erase(s.begin(), std::find_if(s.begin(), s.end(), [](unsigned char ch){ return !std::isspace(ch); })); }
inline void rtrim(Str& s){ s.erase(std::find_if(s.rbegin(), s.rend(), [](unsigned char ch){ return !std::isspace(ch); }).base(), s.end()); }
inline void trim(Str& s){ ltrim(s); rtrim(s); }
...
std::map<Str, double> loadCSVToMap() {
    ...
    while (std::getline(file, line)) {
        ...
        if (pos_sharp != Str::npos) raw.erase(pos_sharp);
        auto pos_dbl = raw.find("//");
        if (pos_dbl != Str::npos) raw.erase(pos_dbl);
